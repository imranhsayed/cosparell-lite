 ...
 */
if ( class_exists( 'cosparell_Loader' ) && defined( 'cosparell_PLUGIN_MODE' ) && cosparell_PLUGIN_MODE == true ) {
  
  add_filter( 'cosparell_theme_mode', '__return_false', 999 );
  
  function cosparell_conflict_notice() {
  ...
  add_action( 'admin_notices', 'cosparell_conflict_notice' );
 ...
 */
if ( ! class_exists( 'cosparell_Loader' ) ) {

  class cosparell_Loader {
    ...
    private function load_languages() {
       ...
       * @since     2.1.3
       */
      define( 'cosparell_PLUGIN_MODE', strpos( dirname( __FILE__ ), 'plugins' . DIRECTORY_SEPARATOR . basename( dirname( __FILE__ ) ) ) !== false ? true : false );
       ...
       * @updated   2.4.1
       */
      if ( cosparell_PLUGIN_MODE ) {
        
        define( 'cosparell_LANG_DIR', trailingslashit( dirname( plugin_basename( __FILE__ ) ) ) . trailingslashit( 'languages' ) );
        
      } else {
      
        if ( apply_filters( 'cosparell_child_theme_mode', false ) == true ) {
        
          $path = ltrim( end( @explode( get_stylesheet(), str_replace( '\\', '/', dirname( __FILE__ ) ) ) ), '/' );
          define( 'cosparell_LANG_DIR', trailingslashit( trailingslashit( get_stylesheet_directory() ) . $path ) . trailingslashit( 'languages' ) . 'cosparell' );
          
        } else {
        
          $path = ltrim( end( @explode( get_template(), str_replace( '\\', '/', dirname( __FILE__ ) ) ) ), '/' );
          define( 'cosparell_LANG_DIR', trailingslashit( trailingslashit( get_template_directory() ) . $path ) . trailingslashit( 'languages' ) . 'cosparell' );
    ...
    public function load_textdomain() {
      ...
      if ( cosparell_PLUGIN_MODE ) {
        ...
        load_plugin_textdomain( 'option-tree', false, cosparell_LANG_DIR );
        
      } else {
      
        load_theme_textdomain( 'option-tree', cosparell_LANG_DIR );
     ...
     * are prefixed with 'cosparell_' to avoid any naming collisions.
     *
     * @return    void
     *
     * @access    private
    ...
    private function constants() {
       ...
       */
      define( 'cosparell_VERSION', '2.5.1' );
       ...
       * @since     2.0
       */
      define( 'cosparell_THEME_MODE', apply_filters( 'cosparell_theme_mode', false ) );
       ...
       * @since     2.0.15
       */
      define( 'cosparell_CHILD_THEME_MODE', apply_filters( 'cosparell_child_theme_mode', false ) );
       ...
       * @since     2.0
       */
      define( 'cosparell_SHOW_PAGES', apply_filters( 'cosparell_show_pages', true ) );
       ...
       * @since     2.1
       */
      define( 'cosparell_SHOW_OPTIONS_UI', apply_filters( 'cosparell_show_options_ui', true ) );
       ...
       * @since     2.1
       */
      define( 'cosparell_SHOW_SETTINGS_IMPORT', apply_filters( 'cosparell_show_settings_import', true ) );
       ...
       * @since     2.1
       */
      define( 'cosparell_SHOW_SETTINGS_EXPORT', apply_filters( 'cosparell_show_settings_export', true ) );
       ...
       * @since     2.0.10
       */
      define( 'cosparell_SHOW_NEW_LAYOUT', apply_filters( 'cosparell_show_new_layout', true ) );
       ...
       * @since     2.1
       */
      define( 'cosparell_SHOW_DOCS', apply_filters( 'cosparell_show_docs', true ) );
       ...
       * @since     2.1
       */
      define( 'cosparell_USE_THEME_OPTIONS', apply_filters( 'cosparell_use_theme_options', true ) );
       ...
       * @since     2.0
       */
      define( 'cosparell_META_BOXES', apply_filters( 'cosparell_meta_boxes', true ) );
       ...
       * @since     2.0
       */
      define( 'cosparell_ALLOW_UNFILTERED_HTML', apply_filters( 'cosparell_allow_unfiltered_html', false ) );
       ...
       * @since     2.4.0
       */
      define( 'cosparell_POST_FORMATS', apply_filters( 'cosparell_post_formats', false ) );
      
      /**
       * Check if in theme mode.
       *
       * If cosparell_THEME_MODE and cosparell_CHILD_THEME_MODE is false, set the 
       * directory path & URL like any other plugin. Otherwise, use 
       * the parent or child themes root directory. 
       *
       * @since     2.0
       */
      if ( false == cosparell_THEME_MODE && false == cosparell_CHILD_THEME_MODE ) {
        define( 'cosparell_DIR', plugin_dir_path( __FILE__ ) );
        define( 'cosparell_URL', plugin_dir_url( __FILE__ ) );
      } else {
        if ( true == cosparell_CHILD_THEME_MODE ) {
          $path = ltrim( end( @explode( get_stylesheet(), str_replace( '\\', '/', dirname( __FILE__ ) ) ) ), '/' );
          define( 'cosparell_DIR', trailingslashit( trailingslashit( get_stylesheet_directory() ) . $path ) );
          define( 'cosparell_URL', trailingslashit( trailingslashit( get_stylesheet_directory_uri() ) . $path ) );
        } else {
          $path = ltrim( end( @explode( get_template(), str_replace( '\\', '/', dirname( __FILE__ ) ) ) ), '/' );
          define( 'cosparell_DIR', trailingslashit( trailingslashit( get_template_directory() ) . $path ) );
          define( 'cosparell_URL', trailingslashit( trailingslashit( get_template_directory_uri() ) . $path ) );
        }
      }
       ...
       */
      if ( true == cosparell_CHILD_THEME_MODE ) {
        define( 'cosparell_THEME_URL', get_stylesheet_directory_uri() );
      } else {
        define( 'cosparell_THEME_URL', get_template_directory_uri() );
      }
    ...
    private function admin_includes() {
      ...
      if ( cosparell_META_BOXES == true ) {
        $files[] = 'ot-meta-box-api';
      }
      
      /* include the post formats api */
      if ( cosparell_META_BOXES == true && cosparell_POST_FORMATS == true ) {
        $files[] = 'ot-post-formats-api';
      }
      
      /* include the settings & docs pages */
      if ( cosparell_SHOW_PAGES == true ) {
      ...
      foreach ( $files as $file ) {
        $this->load_file( cosparell_DIR . "includes" . DIRECTORY_SEPARATOR . "{$file}.php" );
      }
      
      /* Registers the Theme Option page */
      add_action( 'init', 'cosparell_register_theme_options_page' );
      
      /* Registers the Settings page */
      if ( cosparell_SHOW_PAGES == true ) {
        add_action( 'init', 'cosparell_register_settings_page' );
      }
    ...
    private function hooks() {
      ...
      if ( function_exists( 'cosparell_maybe_migrate_settings' ) )
        add_action( 'init', 'cosparell_maybe_migrate_settings', 1 );
      
      // Attempt to migrate the Options
      if ( function_exists( 'cosparell_maybe_migrate_options' ) )
        add_action( 'init', 'cosparell_maybe_migrate_options', 1 );
      
      // Attempt to migrate the Layouts
      if ( function_exists( 'cosparell_maybe_migrate_layouts' ) )
        add_action( 'init', 'cosparell_maybe_migrate_layouts', 1 );

      /* load the Meta Box assets */
      if ( cosparell_META_BOXES == true ) {
      
        /* add scripts for metaboxes to post-new.php & post.php */
        add_action( 'admin_print_scripts-post-new.php', 'cosparell_admin_scripts', 11 );
        add_action( 'admin_print_scripts-post.php', 'cosparell_admin_scripts', 11 );
              
        /* add styles for metaboxes to post-new.php & post.php */
        add_action( 'admin_print_styles-post-new.php', 'cosparell_admin_styles', 11 );
        add_action( 'admin_print_styles-post.php', 'cosparell_admin_styles', 11 );
      
      }
      
      /* Adds the Theme Option page to the admin bar */
      add_action( 'admin_bar_menu', 'cosparell_register_theme_options_admin_bar_menu', 999 );
      
      /* prepares the after save do_action */
      add_action( 'admin_init', 'cosparell_after_theme_options_save', 1 );
      
      /* default settings */
      add_action( 'admin_init', 'cosparell_default_settings', 2 );
      
      /* add xml to upload filetypes array */
      add_action( 'admin_init', 'cosparell_add_xml_to_upload_filetypes', 3 );
      
      /* import */
      add_action( 'admin_init', 'cosparell_import', 4 );
      
      /* export */
      add_action( 'admin_init', 'cosparell_export', 5 );
      
      /* save settings */
      add_action( 'admin_init', 'cosparell_save_settings', 6 );
      
      /* save layouts */
      add_action( 'admin_init', 'cosparell_modify_layouts', 7 );
      
      /* create media post */
      add_action( 'admin_init', 'cosparell_create_media_post', 8 );
      
      /* global CSS */
      add_action( 'admin_head', array( $this, 'global_admin_css' ) );

      /* Google Fonts front-end CSS */
      add_action( 'wp_enqueue_scripts', 'cosparell_load_google_fonts_css', 1 );
 
      /* dynamic front-end CSS */
      add_action( 'wp_enqueue_scripts', 'cosparell_load_dynamic_css', 999 );

      /* insert theme CSS dynamically */
      add_action( 'cosparell_after_theme_options_save', 'cosparell_save_css' );
      ...
      add_action( 'wp_ajax_cosparell_google_font', array( $this, 'retrieve_google_font' ) );
    ...
    public function global_admin_css() {
      ...
      <style>
        @font-face {
          ...
          src:url("' . cosparell_URL . 'assets/fonts/option-tree-font.eot");
          src:url("' . cosparell_URL . 'assets/fonts/option-tree-font.eot?#iefix") format("embedded-opentype"),
            url("' . cosparell_URL . 'assets/fonts/option-tree-font.woff") format("woff"),
            url("' . cosparell_URL . 'assets/fonts/option-tree-font.ttf") format("truetype"),
            url("' . cosparell_URL . 'assets/fonts/option-tree-font.svg#option-tree-font") format("svg");
          font-weight: normal;
          font-style: normal;
        }
        #adminmenu #toplevel_page_ot-settings .menu-icon-generic div.wp-menu-image:before,
    ...
    public function add_section() {
      echo cosparell_sections_view( cosparell_settings_id() . '[sections]', $_REQUEST['count'] );
      die();
    }
    ...
    public function add_setting() {
      echo cosparell_settings_view( $_REQUEST['name'], $_REQUEST['count'] );
      die();
    }
    ...
    public function add_list_item_setting() {
      echo cosparell_settings_view( $_REQUEST['name'] . '[settings]', $_REQUEST['count'] );
      die();
    }
    ...
    public function add_the_contextual_help() {
      echo cosparell_contextual_help_view( $_REQUEST['name'], $_REQUEST['count'] );
      die();
    }
    ...
    public function add_choice() {
      echo cosparell_choices_view( $_REQUEST['name'], $_REQUEST['count'] );
      die();
    }
    ...
    public function add_layout() {
      echo cosparell_layout_view( $_REQUEST['count'] );
      die();
    }
    ...
    public function add_list_item() {
      cosparell_list_item_view( $_REQUEST['name'], $_REQUEST['count'], array(), $_REQUEST['post_id'], $_REQUEST['get_option'], unserialize( cosparell_decode( $_REQUEST['settings'] ) ), $_REQUEST['type'] );
      die();
    }
    ...
    public function add_social_links() {
      cosparell_social_links_view( $_REQUEST['name'], $_REQUEST['count'], array(), $_REQUEST['post_id'], $_REQUEST['get_option'], unserialize( cosparell_decode( $_REQUEST['settings'] ) ), $_REQUEST['type'] );
      die();
    }
    ...
    public function shortcode( $settings, $post ) {
      ...
      if ( ! is_object( $post ) ) {
        ...
        if ( $post_id == 0 && function_exists( 'cosparell_get_media_post_ID' ) ) {
      ...
      $settings['cosparell_gallery'] = array( 'shortcode' => "[gallery id='{$settings['post']['id']}']" );
    ...
    public function retrieve_google_font() {
      ...
      if ( isset( $_POST['field_id'], $_POST['family'] ) ) {
        ...
        cosparell_fetch_google_fonts();
        
        echo json_encode( array(
          'variants' => cosparell_recognized_google_font_variants( $_POST['field_id'], $_POST['family'] ),
          'subsets'  => cosparell_recognized_google_font_subsets( $_POST['field_id'], $_POST['family'] )
        ) );
        exit();
    ...
    public function change_image_button( $translation, $text, $domain ) {
      ...
      if ( $pagenow == apply_filters( 'cosparell_theme_options_parent_slug', 'themes.php' ) && 'default' == $domain && 'Insert into post' == $text ) {
        
        // Once is enough.
        remove_filter( 'gettext', array( $this, 'cosparell_change_image_button' ) );
        return apply_filters( 'cosparell_upload_text', __( 'Send to OptionTree', 'option-tree' ) );
   ...
   * @since     2.0
   */
  $cosparell_loader = new cosparell_Loader();
