 ...
 */
if ( ! class_exists( 'cosparell_Cleanup' ) ) {

  class cosparell_Cleanup {
    ...
    function __construct() {
      ...
      add_action( 'cosparell_pre_consolidate_posts', array( $this, 'increase_timeout' ) );
    ...
    public function maybe_cleanup() {
      global $wpdb, $table_prefix, $cosparell_maybe_cleanup_posts, $cosparell_maybe_cleanup_table;
      ...
      $cosparell_maybe_cleanup_posts = count( $posts ) > 1;
      $cosparell_maybe_cleanup_table = count( $table ) == 1;
      $page = isset( $_GET['page'] ) ? $_GET['page'] : '';

      if ( ! $cosparell_maybe_cleanup_posts && ! $cosparell_maybe_cleanup_table && $page == 'ot-cleanup' ) {
        wp_redirect( apply_filters( 'cosparell_theme_options_parent_slug', 'themes.php' ) . '?page=' . apply_filters( 'cosparell_theme_options_menu_slug', 'cosparell-theme-options' ) );
        exit;
      }

      if ( $cosparell_maybe_cleanup_posts || $cosparell_maybe_cleanup_table ) {
        ...
        $theme_check_bs( apply_filters( 'cosparell_cleanup_page_title', __( 'OptionTree Cleanup', 'option-tree' ) ), apply_filters( 'cosparell_cleanup_menu_title', __( 'OptionTree Cleanup', 'option-tree' ) ), 'edit_theme_options', 'ot-cleanup', array( $this, 'options_page' ) );
    ...
    public function cleanup_notice() {
      ...
      if ( get_current_screen()->id != 'appearance_page_ot-cleanup' )
        echo '<div class="update-nag">' . sprintf( __( 'OptionTree has outdated data that should be removed. Please go to %s for more information.', 'option-tree' ), sprintf( '<a href="%s">%s</a>', admin_url( 'themes.php?page=ot-cleanup' ), apply_filters( 'cosparell_cleanup_menu_title', __( 'OptionTree Cleanup', 'option-tree' ) ) ) ) . '</div>';
    ...
    public function options_page() {
      ...
      echo '<div class="wrap">';
        ...
        echo '<h2>' . apply_filters( 'cosparell_cleanup_page_title', __( 'OptionTree Cleanup', 'option-tree' ) ) . '</h2>';

        if ( $cosparell_maybe_cleanup_posts ) {
          ...
          echo $safe_mode ?  '<p>' . sprintf( __( '%s Your server is running in safe mode. Which means this page will automatically reload after deleting %s posts, you can filter this number using %s if your server is having trouble processing that many at one time.', 'option-tree' ), '<strong>Note</strong>:', apply_filters( 'cosparell_consolidate_posts_reload', 500 ), '<tt>cosparell_consolidate_posts_reload</tt>' ) . '</p>' : '';
          ...
          if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'consolidate-posts' ) ) {
            ...
            do_action( 'cosparell_pre_consolidate_posts' );
            ...
            foreach( $posts as $post ) {
              ...
              if ( $safe_mode && $count > apply_filters( 'cosparell_consolidate_posts_reload', 500 ) ) {
                echo '<br />' . __( 'Reloading...', 'option-tree' );
                echo '
                <script>
                  setTimeout( cosparell_script_reload, 3000 )
                  function cosparell_script_reload() {
        ...
        if ( $cosparell_maybe_cleanup_table ) {

          $table_name = $table_prefix . 'option_tree';

          echo $cosparell_maybe_cleanup_posts ? '<hr />' : '';
