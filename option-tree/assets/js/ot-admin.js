  ...
  cosparell_UI = {
    ...
    init_add: function() {
      ...
      $(document).on('click', '.option-tree-list-item-setting-add', function(e) {
        ...
        cosparell_UI.add(this,'list_item_setting');
      });
    },
    init_edit: function() {
      $(document).on('click', '.option-tree-setting-edit', function(e) {
        ...
        if ( $(this).parents().hasClass('option-tree-setting-body') ) {
          cosparell_UI.init_remove_active($(this),'child');
          cosparell_UI.init_hide_body($(this),'child');
        } else {
          cosparell_UI.init_remove_active($(this),'parent');
          cosparell_UI.init_hide_body($(this), 'parent');
        }
        $(this).toggleClass('active');
        cosparell_UI.init_hide_body($(this), 'toggle');
      });
    },
    init_remove: function() {
      $(document).on('click', '.option-tree-setting-remove', function(event) {
        ...
        if (agree) {
          ...
          cosparell_UI.remove(this);
          setTimeout( function() { 
            cosparell_UI.update_ids(list); 
          }, 200 );
        }
        return false;
      });
        ...
        $.ajax({
          ...
          complete: function( data ) {
            if ( type == 'choice' || type == 'list_item_setting' ) {
              cosparell_UI.init_remove_active(list,'child-add');
              cosparell_UI.init_hide_body(list,'child-add');
            } else {
              cosparell_UI.init_remove_active();
              cosparell_UI.init_hide_body();
            }
            ...
            cosparell_UI.init_sortable(listItem);
            cosparell_UI.init_select_wrapper(listItem);
            cosparell_UI.init_numeric_slider(listItem);
            cosparell_UI.parse_condition();
    ...
    update_id: function(e) {
      ...
      this.timer = setTimeout( function() {
        cosparell_UI.update_ids($(e).parents('ul'));
      }, 100);
      return true;
    },
    ...
    parse_condition: function() {
      $( '.format-settings[id^="setting_"][data-condition]' ).each(function() {
        ...
        var passed;
        var conditions = cosparell_UI.match_conditions( $( this ).data( 'condition' ) );
        var operator = ( $( this ).data( 'operator' ) || 'and' ).toLowerCase();

        $.each( conditions, function( index, condition ) {

          var target   = $( '#setting_' + condition.check );
          var targetEl = !! target.length && target.find( cosparell_UI.condition_objects() ).first();
    ...
    init_conditions: function() {
      ...
      $('.format-settings[id^="setting_"]').on( 'change.conditionals, keyup.conditionals', cosparell_UI.condition_objects(), function(e) {
        if (e.type === 'keyup') {
    ...
    init_upload: function() {
      $(document).on('click', '.cosparell_upload_media', function() {
        ...
        if ( window.wp && wp.media ) {
          window.cosparell_media_frame = window.cosparell_media_frame || new wp.media.view.MediaFrame.Select({
          ...
          window.cosparell_media_frame.on('select', function() {
            var attachment = window.cosparell_media_frame.state().get('selection').first(), 
            ...
            window.cosparell_media_frame.off('select');
          }).open();
        } else {
          ...
          window.send_to_editor = function(html) {
            ...
            if (href.match(image) && cosparell_UI.url_exists(href)) {
              btnContent += '<div class="option-tree-ui-image-wrap"><img src="'+href+'" alt="" /></div>';
            }
            btnContent += '<a href="javascript:(void);" class="option-tree-ui-remove-media option-tree-ui-button button button-secondary light" title="'+option_tree.remove_media_text+'"><span class="icon ot-icon-minus-circle"></span>'+option_tree.remove_media_text+'</a>';
            $('#'+field_id).val(href);
            ...
            $('#'+field_id).parent().parent('div').append('<div class="option-tree-ui-media-wrap" id="'+field_id+'_media" />');
            $('#'+field_id+'_media').append(btnContent).slideDown();
            cosparell_UI.fix_upload_parent();
            tb_remove();
    ...
    init_upload_remove: function() {
      $(document).on('click', '.option-tree-ui-remove-media', function(event) {
        ...
        var agree = confirm(option_tree.remove_agree);
        if (agree) {
          cosparell_UI.remove_image(this);
          return false;
        }
        return false;
      });
    ...
    init_upload_fix: function(elm) {
      ...
      if ( val !== '' && ( typeof src == 'undefined' || src == false ) && cosparell_UI.url_exists(val) ) {
        var image = /\.(?:jpe?g|png|gif|ico)$/i;
        if (val.match(image)) {
      ...
      } else if ( val == '' || ! cosparell_UI.url_exists(val) ) {
    ...
    fix_upload_parent: function() {
      $('.option-tree-ui-upload-input').not('.ot-upload-attachment-id').on('focus blur', function(){
        ...
        cosparell_UI.init_upload_fix(this);
      });
    },
...
!function ($) {
  ...
  cosparell_gallery = {
    ...
    frame: function (elm) {
      ...
      this._frame.on('update', function () {
        var controller = cosparell_gallery._frame.states.get('gallery-edit')
  ...
  , select: function (elm) {
      var input = $(elm).parents('.format-setting-inner').children('.ot-gallery-value')
        ...
        , shortcode = wp.shortcode.next('gallery', ( ids ? _shortcode : wp.media.view.settings.cosparell_gallery.shortcode ) )
  ...
  , open: function (elm) {
      ...
      cosparell_gallery.frame(elm).open()
  ...
  // Gallery delete
  $(document).on('click.cosparell_gallery.data-api', '.ot-gallery-delete', function (e) {
    e.preventDefault()
    cosparell_gallery.remove($(this))
  })
  
  // Gallery edit
  $(document).on('click.cosparell_gallery.data-api', '.ot-gallery-edit', function (e) {
    e.preventDefault()
    cosparell_gallery.open($(this))
  })
      ...
      if ( $(this).find('.type-tab').length ) {
        ...
        $(this).find('.ot-metabox-tabs').tabs({
          activate: function( event, ui ) {
            ...
            cosparell_UI.css_editor_mode();
            cosparell_UI.javascript_editor_mode();
          }
        })
      ...
      $('.wrap').on('mouseup', function () {
        ...
        $('#' + parent_id + ' input[type="radio"]').each( function () {
          ...
          if ( $(this).is(':checked') ) {
            ...
            cosparell_checked_radios[$(this).attr('name')] = $(this).val()
          
          }
          
          // write to the object
          $(document).data('cosparell_checked_radios', cosparell_checked_radios)
        ...
        setTimeout( function () {
          ...
          var checked = $(document).data('cosparell_checked_radios')
...
;(function ( $, window, document, undefined ) {
  ...
  $.cosparell_ParseColorValue = function( val ) {
  ...
  $.fn.cosparell_wpColorPicker = function() {
    ...
    return this.each(function() {
      ...
      if( $this.data('rgba') !== false ) {
        ...
        // parse value
        var picker = $.cosparell_ParseColorValue( $this.val() );
            ...
            $alpha_slider.slider({
              ...
              create: function() {
                ...
                $container.on('click', '.wp-picker-default', function() {
                  ...
                  var default_picker = $.cosparell_ParseColorValue( $this.data('default-color') ),
  ...
  $(document).ready( function(){
    $('.hide-color-picker.ot-colorpicker-opacity').cosparell_wpColorPicker();
  });
