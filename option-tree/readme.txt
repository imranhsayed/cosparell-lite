...
For a list of all the OptionTree UI display filters refer to the `demo-functions.php` file found in the `/assets/cosparell/` directory of this plugin. This file is the starting point for developing themes with Theme Mode.
...
* Hotfix - Force the `cosparell_google_fonts` array to be rebuilt when switching between themes.
...
* Added filter `cosparell_dequeue_jquery_ui_css_screen_ids` to dequeue `jquery-ui-css` by screen ID.
* Added filter `cosparell_on_off_switch_on_value` to filter the value of the On button. Contributors via github @BassemN, and @valendesigns.
* Added filter `cosparell_on_off_switch_on_label` to filter the label of the On button. Contributors via github @BassemN, and @valendesigns.
* Added filter `cosparell_on_off_switch_off_value` to filter the value of the Off button. Contributors via github @BassemN, and @valendesigns.
* Added filter `cosparell_on_off_switch_off_label` to filter the label of the Off button. Contributors via github @BassemN, and @valendesigns.
* Added filter `cosparell_on_off_switch_width` to filter the width of the On/Off switch.
* Added filter `cosparell_type_date_picker_readonly` to filter the addition of the readonly attribute.
* Added filter `cosparell_type_date_time_picker_readonly` to filter the addition of the readonly attribute.
* Added filter `cosparell_admin_menu_priority` to filter the `admin_menu` action hook priority.
...
* Changed where `cosparell_css_file_paths` is saved when `is_multisite` for better `dynamic.css` file support.
...
* Hotfix - Fixed an issue where `cosparell_get_media_post_ID()` was never able to set the value of the `cosparell_media_post_ID` option because it was already set to empty. Causing the `cosparell_create_media_post()` function to create multiple media posts.

= 2.4.5 =
* Hotfix - Fixed an issue where `cosparell_get_media_post_ID()` was setting the value of the `cosparell_media_post_ID` option to `null`. Causing the `cosparell_create_media_post()` function to create multiple media posts. A clean up script will be added to `2.5.0`.
...
* Hotfix - Fixed the `cosparell_line_height_range_interval` filter being misnamed as `cosparell_line_height_unit_type`. Contributors via github @youri--.
...
* Hotfix - Check for `post_title` instead of `post_name` in `cosparell_get_media_post_ID()`. Contributors via github @clifgriffin.
* Hotfix - Store the return value of `cosparell_get_media_post_ID()` in the options table as `cosparell_media_post_ID`.
...
* Hotfix - Added filter `cosparell_override_forced_textarea_simple` to allow the Textarea option type to be moved in the DOM and not replaced with the Textarea Simple option type in meta boxes and list items.
...
= 2.4.0 =
* Added filter 'cosparell_post_formats' which loads meta boxes specifically for post formats.
...
* Added filter 'cosparell_type_select_choices' to dynamically change select choices. Contributors via github @maimairel and @valendesigns.
...
* Added filter `cosparell_recognized_font_sizes` to dynamically change the font sizes by field ID.
* Added filter `cosparell_recognized_letter_spacing` to dynamically change the letter spacing by field ID.
* Added filter `cosparell_recognized_line_heights` to dynamically change the line heights by field ID.
...
* Hotfix - Added filter `cosparell_migrate_settings_id` to migrate themes that used `cosparell_settings` and now use a custom settings ID.
* Hotfix - Added filter `cosparell_migrate_options_id` to migrate themes that used `option_tree` and now use a custom options ID.
* Hotfix - Added filter `cosparell_migrate_layouts_id` to migrate themes that used `option_tree_layouts` and now use a custom layouts ID.

= 2.3.2 =
* Hotfix - Fixed an issue with the `cosparell_create_media_post` function creating multiple `option-tree` posts.
...
* Hotfix - Added filter `cosparell_type_date_picker_date_format` to change the date format of the Date Picker option type.
* Hotfix - Added filter `cosparell_type_date_time_picker_date_format` to change the date format of the Date Time Picker option type.
...
* Added filter 'cosparell_list_item_title_label' to change the label for a List Item's required title field.
* Added filter 'cosparell_list_item_title_desc' to change the description for a List Item's required title field.
* Added filter 'cosparell_options_id' to change the 'option_tree' option ID to a unique value.
* Added filter 'cosparell_settings_id' to change the 'cosparell_settings' option ID to a unique value.
* Added filter 'cosparell_layouts_id' to change the 'option_tree_layouts' option ID to a unique value.
* Added filter 'cosparell_header_logo_link' to change the logo link inside the header of OptionTree.
* Added filter 'cosparell_header_version_text' to change the version text inside the header of OptionTree.
* Added action 'cosparell_header_list' to add additional theme specific list items to the header of OptionTree.
* Added filter 'cosparell_upload_text' to change the "Send to OptionTree" text.
...
* Fixed a bug that kept the UI from displaying when using the `cosparell_type_background_size_choices` filter.
...
* Added filter 'cosparell_type_radio_image_src' which allows the Radio Image option type source URI to be changed. Contributors via github @bitcommit.
...
* Added filter 'cosparell_recognized_background_fields' to show/hide fields for background option types.
* Added filter 'cosparell_filter_description' that allows the theme option descriptions to be filtered before being displayed.
...
* Added filter 'cosparell_type_category_checkbox_query' which allows you to filter the get_categories() args for Category Checkbox.
* Added filter 'cosparell_type_category_select_query' which allows you to filter the get_categories() args for Category Select.
* Added filter 'cosparell_type_taxonomy_checkbox_query' which allows you to filter the get_categories() args for Taxonomy Checkbox.
* Added filter 'cosparell_type_taxonomy_select_query' which allows you to filter the get_categories() args for Taxonomy Select.
* Added the 'cosparell_echo_option' function. Contributors via github @joshlevinson.
* Added filter 'cosparell_theme_options_contextual_help' which allows you to filter the Contextual Help on the Theme Options page.
* Added filter 'cosparell_theme_options_sections' which allows you to filter the Sections on the Theme Options page. Contributors via github @joshlevinson.
* Added filter 'cosparell_theme_options_settings' which allows you to filter the Settings on the Theme Options page. Contributors via github @joshlevinson.
...
= 2.1.1 =
* Hotfix - The cosparell_SHOW_SETTINGS_EXPORT constant was incorrectly set to false as the default.
...
* Added dynamic replacement of 'cosparell_URL' & 'cosparell_THEME_URL' in the Radio Image source path.
* Make '0' possible as a field value. Validate for empty strings instead of empty(). Contributors via github @maimairel.
* The 'cosparell_theme_options_capability' filter is now working for different capabilities like editor.
* The 'cosparell_display_by_type' filter is now being assigned to a value.
* Added filter 'cosparell_show_options_ui' which allows you to hide the Theme Options UI Builder.
* Added filter 'cosparell_show_settings_import' which allows you to hide the Settings Import options on the Import page.
* Added filter 'cosparell_show_settings_export' which allows you to hide the Settings Export options on the Export page.
* Added filter 'cosparell_show_docs' which allows you to hide the Documentation.
* Added filter 'cosparell_use_theme_options' which allows you to hide the OptionTree Theme Option page (not recommended for beginners).
* Added filter 'cosparell_list_item_description' which allows you to change the default list item description text.
* Added filter 'cosparell_type_custom_post_type_checkbox_query' which allows you to filter the get_posts() args for Custom Post Type Checkbox.
* Added filter 'cosparell_type_custom_post_type_select_query' which allows you to filter the get_posts() args for Custom Post Type Select.
* Added filter 'cosparell_type_page_checkbox_query' which allows you to filter the get_posts() args for Page Checkbox.
* Added filter 'cosparell_type_page_select_query' which allows you to filter the get_posts() args for Page Select.
* Added filter 'cosparell_type_post_checkbox_query' which allows you to filter the get_posts() args for Post Checkbox.
* Added filter 'cosparell_type_post_select_query' which allows you to filter the get_posts() args for Post Select.
...
* Added 'cosparell_before_theme_options_save' action hook.
...
* Fixed missing required arguments in cosparell_Loader::add_layout.
* Removed esc_attr() on font-family check.
* Added a 'cosparell_theme_options_parent_slug' filter in ot-ui-theme-options.php
* Fixed WP_Error from the use of wp_get_remote() instead of file_get_contents().

= 2.0.13 =
* Removed almost all of the Theme Check nag messages when in 'cosparell_theme_mode'.
...
* Added the 'cosparell_after_validate_setting' filter to the validation function.
* Added $field_id to the cosparell_validate_setting() for more precise filtering.
* Added the cosparell_reverse_wpautop() function that you can run input through just incase you need it.
...
= 2.0.5 =
* Change the way the 'cosparell_settings' array validates. Strip out those damn slashes!
...
* Added the simpler cosparell_get_option() function to eventually replace get_option_tree().
